...
// Use useLayoutEffect on client for sync updates to prevent FOUC
...
export function IntegratedHeroGrid({
    ...
    // 🔥 FIX 1: Instant Column Calculation using useMemo (Prevents Flash)
    ...
    useEffect(() => {
        const handleEscape = (e: KeyboardEvent) => {
            if (e.key === "Escape" && selectedProject) handleCloseModal()
            if (e.key === "ArrowLeft" && selectedProject) handleGalleryPrev()
            if (e.key === "ArrowRight" && selectedProject) handleGalleryNext()
        }
    ...
    useEffect(() => {
        const updateColumnCount = () => {
            ...
            else if (window.innerWidth < 1440) newColumnCount = 3
            else if (window.innerWidth < 1920) newColumnCount = 4
    ...
    const renderMedia = (media: ProjectMedia, alt: string, fill = true, className = "") => {
        if (!media || !media.src) return <div className="h-full w-full bg-slate-800" />
    ...
    return (
        <>
            {/* 🔥 FIX 2: CSS Lock - Forces new items to be invisible until Framer Motion takes over */}
            <style jsx global>{`
        .motion-hidden-until-animate { visibility: hidden !important; }
        .motion-hidden-until-animate[data-animated="true"] { visibility: visible !important; }
      `}</style>
            ...
            <section className="relative min-h-screen py-8 pt-12 md:py-16 md:pt-16 lg:pt-16 lg:[@media(pointer:fine)]:pt-40 lg:[@media(pointer:fine)]:py-32">
                <div className="relative mx-auto max-w-[95%] px-4 md:px-6 2xl:max-w-[1800px]">
                    ...
                    <div className="relative mx-auto w-full max-w-[96%] px-4 md:px-8 2xl:max-w-[1800px]">
                        <div className="relative z-10 mb-12 flex flex-col items-center justify-center text-center lg:flex-row lg:justify-between lg:text-left" style={{ animation: "fadeUp 800ms ease-out" }}>
                            <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-center">
                                <div className="relative flex-shrink-0">
                                    <div className="absolute inset-0 rounded-3xl opacity-50" style={{ background: "radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.3), transparent 70%)", filter: "blur(50px)", animation: "pulse 3s ease-in-out infinite" }} />
                                    <div className="relative h-24 w-24 overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl md:h-32 md:w-32 [mask-image:linear-gradient(to_bottom,black_80%,transparent_100%)]">
                                        <Image src={photoSrc || "/placeholder.svg?height=160&width=160"} alt={name} fill className="object-cover" sizes="160px" />
                                    </div>
                                </div>

                                <div className="flex flex-col items-center lg:items-start">
                                    <h1 className="font-serif text-4xl font-bold text-white md:text-6xl whitespace-nowrap">{name}</h1>
                                    <p className="mt-2 font-sans text-xl font-medium text-white md:text-2xl">{title}</p>
                                    <p className="mt-1 font-sans text-sm text-white/60 md:text-base">Helping brands stand out with elegant visuals & motion.</p>
                                </div>
                            </div>
                            ...
                            {!isTouchDevice && brandLogos.length > 0 && (
                                <div className="hidden lg:grid mt-12 grid-cols-2 gap-4 lg:mt-0 lg:flex-shrink-0">
                                    {brandLogos.map((brand, index) => (
                                        ...
                                        >
                                            <Image src={brand.logoSrc || "/placeholder.svg?height=96&width=96"} alt={brand.name} fill className="object-contain p-4 transition-all duration-500" sizes="96px" />
                                        </Link>
                                    ))}
                                </div>
                            )}
                            ...
                            {columns.map((column, columnIndex) => (
                                <div key={columnIndex} className="flex flex-1 flex-col gap-5">
                                    {column.map((project, itemIndex) => {
                                        ...
                                        // Smart Stagger per item in chunk
                                        const staggerDelay = Math.min((columnIndex * 0.1) + (itemIndex * 0.05), 0.5)
                                        ...
                                        return (
                                            <GridItem
                                                ...
                                                isLoaded={isLoaded}
                                                delay={staggerDelay}
                                                handleProjectClick={() => handleProjectClick(project, globalIndex)}
                            ...
                            >
                                <AnimatePresence mode="wait" custom={slideDirection}>
                                    <motion.div
                                        ...
                                        onDragEnd={(e, { offset, velocity }) => {
                                            ...
                                            if (offset.x > swipeThreshold) handleGalleryPrev()
                                            else if (offset.x < -swipeThreshold) handleGalleryNext()
                                        }}
                                    >
                                        {currentMedia && renderMedia(currentMedia, selectedProject.title, true, "transition-transform duration-500")}
                                    </motion.div>
                                </AnimatePresence>
                                ...
                                {selectedProject.gallery && selectedProject.gallery.length > 1 && (
                                    <>
                                        {!isTouchDevice && (
                                            <>
                                                <button onClick={handleGalleryPrev} className="force-arrow absolute left-2 top-1/2 -translate-y-1/2 rounded-full border border-white/30 bg-white/10 p-2 text-white backdrop-blur-xl"><ChevronLeft className="h-5 w-5" /></button>
                                                <button onClick={handleGalleryNext} className="force-arrow absolute right-2 top-1/2 -translate-y-1/2 rounded-full border border-white/30 bg-white/10 p-2 text-white backdrop-blur-xl"><ChevronRight className="h-5 w-5" /></button>
                                            </>
                                        )}
                                        <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2">
                                            {selectedProject.gallery.map((_, idx) => (
                                                <button key={idx} onClick={() => setCurrentGalleryIndex(idx)} className={`force-arrow h-2 w-2 rounded-full transition-all ${idx === currentGalleryIndex ? "bg-white scale-125" : "bg-white/40"}`} />
                                            ))}
                                        </div>
                                        {isTouchDevice && <p className="absolute bottom-10 left-1/2 -translate-x-1/2 text-xs text-white/50 font-sans">Swipe for more</p>}
                                    </>
                                )}
                            </div>

                            <div className="flex w-full flex-col justify-center gap-4 p-6 md:w-1/2 md:p-10">
                                <h2 className="font-serif text-2xl font-bold text-white md:text-3xl">{selectedProject.title}</h2>
                                <span className="w-fit rounded-full bg-white/10 px-3 py-1 font-sans text-xs text-white/70">{selectedProject.category}</span>
                                <p className="font-sans text-sm text-white/80 md:text-base">{selectedProject.description}</p>
                                <div className="font-sans text-xs leading-relaxed text-white/60 md:text-sm">{selectedProject.details}</div>
                                <Link href={selectedProject.detailsLink || `/projects/${selectedProject.id}`} className="group mt-4 flex w-fit items-center gap-2 rounded-full border border-white/30 bg-white/10 px-6 py-3 font-sans text-sm font-medium text-white backdrop-blur-md transition-all hover:scale-105 hover:border-white/50 hover:bg-white/20 hover:shadow-lg hover:shadow-white/10">
                                    Project Details <ExternalLink className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                                </Link>
                            </div>
                        </motion.div>
                    </AnimatePresence>
...
// ----------------------------------------------------------------------
// 🔥 GRID ITEM COMPONENT (Restored TiltCard + Added Animation Fix)
// ----------------------------------------------------------------------
function GridItem({ project, isLoaded, delay, handleProjectClick, setHoveredId, handleImageLoad, videoRefs, isTouchDevice }: any) {
    const ref = useRef<HTMLDivElement>(null)
    const [isAnimating, setIsAnimating] = useState(false)
    const [hasCompletedEntry, setHasCompletedEntry] = useState(false)

    // CSS Lock Trigger: Flips data-animated to "true" when Framer Motion starts
    const handleAnimationStart = useCallback(() => {
        if (!isAnimating) {
            setIsAnimating(true)
            if (ref.current) ref.current.setAttribute("data-animated", "true")
        }
    }, [isAnimating])
    ...
    return (
        <TiltCard>
            <motion.div
                ref={ref}
                data-animated="false"
                className="motion-hidden-until-animate group relative cursor-pointer overflow-hidden rounded-3xl border border-white/20 bg-white/5 shadow-xl backdrop-blur-xl transition-all duration-500 ease-out hover:border-white/40 hover:bg-white/10 hover:shadow-2xl hover:shadow-white/10"
                style={{ aspectRatio: `${project.width} / ${project.height}` }}
                // Framer Motion Animation Props
                initial={{ opacity: 0, y: 30 }}
                animate={isLoaded || hasCompletedEntry ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
                transition={{ duration: 0.6, ease: "easeOut", delay: isLoaded && !hasCompletedEntry ? delay : 0 }}
                onAnimationStart={handleAnimationStart}
                onAnimationComplete={() => isLoaded && setHasCompletedEntry(true)}
            ...
            >
                    ...
                    {project.thumbnail?.type === "video" ? (
                        <>
                            ...
                            <video
                                ...
                                autoPlay loop muted playsInline controlsList="nodownload" disablePictureInPicture draggable={false}
                                onContextMenu={(e) => e.preventDefault()}
                ...
                <div className={`absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-6 transition-all duration-500 ${isTouchDevice ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
